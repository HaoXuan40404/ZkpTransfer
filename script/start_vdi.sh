...
JAVA_OPTS+=" -Dcom.sun.management.jmxremote.port=43464"
JAVA_OPTS+=" -Djava.security.egd=file:/dev/./urandom"
JAVA_OPTS+=" -Xmx1024m -Xms1024m -Xmn512m -Xss512k -XX:MetaspaceSize=256m -XX:MaxMetaspaceSize=256m"
JAVA_OPTS+=" -XX:+HeapDumpOnOutOfMemoryError -XX:HeapDumpPath=${LOG_DIR}/ppcs-modeladm-iss-heap_error.log"
        ...
        do
            ...
            if [ -n "$check_time" ]; then
                ...
                if [[ ${timestamp} -gt ${start_timestamp} ]]; then
                    ...
                    rm -rf logs
                    ln -s ${LOG_DIR} logs
					echo "Iss start Successful!" > success.txt
                    exit 0
                fi
            fi
            sleep 1
