...
import com.webank.ppc.iss.eventsub.UploadSubscribeEvent;
...
public class DatabaseUploadEventConfig {
    ...
    @Bean
    public void registerBcosDatabaseUploadEvent() {
        // 参数设置
        EventSubParams params = new EventSubParams();

        // 最新Event fromBlock设置为"latest"
        BlockChain blockChain = ppcService.getEventStatus(EventEnum.UPLOAD.getValue());
        if(blockChain == null) {
            // 如果未找到从头开始
        ...
        params.addTopic(0, topicTools.stringToTopic("uploadMetaEvent(string,string,string)"));

        // 注册事件
        UploadSubscribeEvent callback = new UploadSubscribeEvent(client, serviceConfig, ppcService);
        ...
        try {
            ...
            log.error("registerBcosDatabaseUploadEvent error, " + e.getMessage());
            throw new PpcException(
                    EnumResponseStatus.FAILURE.getErrorCode(),
                    "registerBcosDatabaseUploadEvent failed:" + e.getMessage());
        }
        eventSubscribe.start();
        String registerId = eventSubscribe.subscribeEvent(params, callback);
        log.info("Register upload event successful, registerId = " + registerId);
    }
}
